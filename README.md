...
Un exemple de l'application consistant en un questionnaire démontrant chaque question type est hébergé en ligne à [survey-wizard-test.netlify.app](https://survey-wizard-test.netlify.app/). Cet exemple utilise les données hardcodées dans l'`initialState` et n'est pas connecté à un back-end.
...
- ont une validation implémentée spécifiquement question type par question type (eg. un email valide doit être fourni pour la question type email où un message d'erreur sera affiché à l'utilisateur).
...
Considérez remplacer la commande (nécessaire pour l'intégration continue avec netlify) `"build": "CI= react-scripts build",` par `"build": "react-scripts build",` dans le fichier [`package.json`](./package.json)

Considérez remplacer le `MemoryRouter` par un `BrowserRouter` dans le fichier [`App.js`](./src/App.js), veuillez consulter la [documentation](https://reactrouter.com/web/api/MemoryRouter) de `react-router-dom` pour voir la différence.

## Exigences d'implémentation relatives à l'API back-end
...
- `src/components` contient
  ...
  - le Composant React principal `Form`. L'état relatif aux réponses et aux timings de réponse est géré dans ce Composant.
...
Pour raison de rétrocompatibilité, les Composants implémentant les questions type **Legal** et **YesNo** sont fonctionnellement identiques.

J'ai implémenté un système d'alertes pour les erreurs, et effectue la validation "à la main" plutôt que d'utiliser `formik` ou `react-hook-form` car j'avais confiance en ma capacité à délivrer une application fonctionnelle en codant cette partie par moi-même alors que je n'étais pas sûr de pouvoir adapter `formik` à mon cas d'utilisation particulier.  
